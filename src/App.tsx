...
export function App() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://localhost:3000/api/matches/${matchId}/join`, {
        method: 'POST',
        headers: {
  ...
  const handleLike = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/matches/${matchId}/like`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ 
          ...
          userName,
          restaurant, // Send full restaurant data for displaying matches
        }),
      });
  ...
  // Show match success screen if there are any matches
  if (matches.length > 0) {
    return (
      <MatchSuccess 
        matches={matches} 
